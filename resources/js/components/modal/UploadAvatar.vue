...
const validateFace = async (image) => {
    // Load models
    await faceapi.nets.ssdMobilenetv1.loadFromUri(getEnv('APP_URL') + '/models/');
    await faceapi.nets.faceLandmark68Net.loadFromUri(getEnv('APP_URL') + '/models/');
    await faceapi.nets.faceRecognitionNet.loadFromUri(getEnv('APP_URL') + '/models/');
